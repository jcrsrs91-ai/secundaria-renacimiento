...
// 3. Add grado/grupo to items.push
const itemsPushOld = `        items.push({
          id: docSnap.id,
          folio,
          alumno,
          concepto,
...
if (!c.includes('grado,\n          grupo,')) {
    c = c.replace(/items\.push\(\{\s*id: docSnap\.id,\s*folio,\s*alumno,\s*concepto,\s*monto,\s*montoNum,\s*fecha,\s*estado: data\.pagoInscripcion \? 'Pagado' : 'Pendiente'\s*\}\);/, itemsPushNew);
}
...
if (!c.includes('placeholder="Buscar por alumno o folio..."')) {
...
if (!c.includes('filteredPagos.map(')) {
...
console.log("Master patch successfully applied");
