...
reg = reg.replace(/\{onPrintConstanciaExtra && \([\s\S]*?<\/button>\s*\)\}/, '');
reg = reg.replace(/\{item\.regularizadas\.length > 0 \? \(/, `{item.regularizadas.length > 0 ? (
                          <div className="space-y-2">
                            {onPrintConstanciaExtra && (
                               ...
                               >
                                 <FileText className="w-3 h-3 mr-1" /> Imprimir Constancia Global
                               </button>
                            )}`);
...
// Let's remove ALL `acreditacion_extraordinario` blocks first.
cp = cp.replace(/\) : type === 'acreditacion_extraordinario' \? \([\s\S]*?<\/p>\n\s*/g, ') : ');

// Now inject it cleanly ONCE right before `inscripcion_primero`
