...
const newEditMapLines = `                    {editingResguardo.articulos.map((art, idx) => (
...
const newFormMapLines = `                    {formData.articulos.map((art, idx) => (
                      <div key={idx} className="flex flex-col gap-2 p-3 bg-slate-50 border border-slate-200 rounded-lg">
                        <div className="flex gap-2 items-center">
                          <div className="w-16">
                            <input type="number" placeholder="Cant" className="w-full rounded-md border-slate-300 text-sm" value={art.cantidad || ''} onChange={(e) => {
...
// Because we splice backward, the earlier indices don't change.
// Wait, we can just slice and concat!
// editingResguardo goes from 1685 to 1731
// formData goes from 1836 to 1916 (which became different after the first splice)
// Let's just do it in one go by building a new array.

let newLines = [];
let i = 0;
while (i < lines.length) {
    if (i === 1685) {
        newLines.push(...newEditMapLines);
        i = 1731 + 1; // skip up to 1731
    } else if (i === 1836) {
        newLines.push(...newFormMapLines);
        i = 1916 + 1; // skip up to 1916
    } else {
...
console.log('Successfully written via exact array indices');
