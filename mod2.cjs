...
// Replace Chunk 2: lines 1696 to 1742
const chunk2 = `                    {editingResguardo.articulos.map((art, idx) => (
...
// Replace Chunk 1: lines 1847 to 1927
const chunk1 = `                    {formData.articulos.map((art, idx) => (
                      <div key={idx} className="flex flex-col gap-2 p-3 bg-slate-50 border border-slate-200 rounded-lg">
                        <div className="flex gap-2 items-center">
                          <div className="w-16">
                            <input type="number" placeholder="Cant" className="w-full rounded-md border-slate-300 text-sm" value={art.cantidad || ''} onChange={(e) => {
...
// Line numbers are 0-indexed in array.
// Chunk 1 is 1847 to 1927 => indices 1846 to 1926
// Chunk 2 is 1696 to 1742 => indices 1695 to 1741

// Replace chunk 1 first (higher lines so it doesn't shift chunk 2)
lines.splice(1846, 1926 - 1846 + 1, chunk1);

// Replace chunk 2
lines.splice(1695, 1741 - 1695 + 1, chunk2);

fs.writeFileSync('src/pages/dashboard/Contraloria.jsx', lines.join('\n'));
console.log('UI blocks updated successfully.');
