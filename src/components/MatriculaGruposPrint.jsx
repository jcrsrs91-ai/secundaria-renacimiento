...
export default function MatriculaGruposPrint({ alumnos = [], onClose }) {
  ...
  const renderRow = (label, key, isTotal = false, isGlobal = false, isGroup = false) => {
    ...
    return (
      <tr className={baseClasses} key={key}>
        <td colSpan={isTotal || isGlobal ? 2 : 1} className={`px-3 py-2 border-r border-slate-200 print:border-slate-400 print:px-1 print:py-1 ${isGroup ? 'pl-6' : ''}`}>{label}</td>
  ...
  const renderTurnoWithGroups = (grado, turno) => {
    const key = `${grado}-${turno}`;
    const d = matriculaData[key];
    ...
    grupos.forEach(grupo => {
       ...
       if (matriculaData[groupKey]) {
          rows.push(renderRow(`Grupo ${grupo}`, groupKey, false, false, true));
       }
    });
    
    rows.push(renderRow(`Total ${turno}`, key, false, false, false));
    return rows;
  };
