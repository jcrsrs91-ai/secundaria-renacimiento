...
export default function AprobacionPrint({ activos, materiasPorGrado, onClose }) {
  ...
  const chartData = useMemo(() => {
    return [
      {
        name: '1er Grado',
        Regulares: stats['1er Grado']?.total.regT || 0,
        Irregulares: stats['1er Grado']?.total.irrT || 0,
        'No Acreditados': stats['1er Grado']?.total.noaT || 0,
      },
      {
        name: '2do Grado',
        Regulares: stats['2do Grado']?.total.regT || 0,
        Irregulares: stats['2do Grado']?.total.irrT || 0,
        'No Acreditados': stats['2do Grado']?.total.noaT || 0,
      },
      {
        name: '3er Grado',
        Regulares: stats['3er Grado']?.total.regT || 0,
        Irregulares: stats['3er Grado']?.total.irrT || 0,
        'No Acreditados': stats['3er Grado']?.total.noaT || 0,
      }
    ];
  }, [stats]);
    ...
    const renderRow = (label, row, isTotal = false) => {
      ...
      return (
        <tr key={label} className={`text-center ${isTotal ? 'bg-indigo-50 font-bold border-t-2 border-indigo-200' : 'border-b border-slate-100 hover:bg-slate-50'}`}>
          <td className="py-2 px-1 font-bold text-slate-700 print:py-1">{label}</td>
          <td className="py-2 px-1 print:py-1">{row.extH}</td>
          <td className="py-2 px-1 print:py-1">{row.extM}</td>
          <td className="py-2 px-1 font-bold text-slate-800 bg-slate-50 print:bg-transparent print:py-1">{row.extT}</td>
          <td className="py-2 px-1 print:py-1">{row.regH}</td>
          <td className="py-2 px-1 print:py-1">{row.regM}</td>
          <td className="py-2 px-1 font-bold text-emerald-600 bg-emerald-50/50 print:bg-transparent print:py-1">{row.regT}</td>
          <td className="py-2 px-1 print:py-1">{row.irr1}</td>
          ...
          <td className={`py-2 px-1 print:py-1 font-bold ${row.irrT + row.noaT > 0 ? 'text-red-500' : 'text-emerald-500'}`}>{pct}</td>
          <td className={`py-2 px-1 print:py-1 font-bold ${comprobacion === row.extT ? 'text-slate-700' : 'text-red-600'}`}>{comprobacion}</td>
        </tr>
      );
    };
