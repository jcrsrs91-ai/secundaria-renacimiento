  ...
  return (
    <div className="print-concentrado-only">
      ...
      <div className="landscape-page">
        ...
        <table className="w-full text-[10px] border border-black mb-6">
          <thead>
            <tr className="bg-slate-200">
              <th className="border border-black p-1 text-center w-6">No.</th>
              <th className="border border-black p-1 text-left w-64">Nombre del Alumno</th>
              {materias.map(mat => (
                <th key={mat.id} className="border border-black p-1 text-center truncate overflow-hidden max-w-[60px]" title={mat.name}>
            ...
            {alumnos.map((al, idx) => {
              return (
                <tr key={al.id}>
                  ...
                  <td className="border border-black p-1 font-medium truncate uppercase">{al.apellidoPaterno} {al.apellidoMaterno} {al.nombres}</td>
                  {materias.map(mat => {
                    ...
                    return (
                      <td key={mat.id} className={`border border-black p-1 text-center font-bold ${isFailing ? 'text-red-600' : ''}`}>
                        {val !== null ? val.toFixed(1) : '-'}
                      </td>
                    );
                  })}
                  <td className="border border-black p-1 text-center font-black bg-slate-100">{getPromedioGeneralAlumno(al)}</td>
                </tr>
              );
            })}
          </tbody>
          <tfoot>
            <tr className="bg-slate-100">
              <td colSpan="2" className="border border-black p-1 text-right font-black uppercase">Promedio Grupal por Materia:</td>
              {materias.map(mat => {
                const promGral = countPorMateria[mat.id] > 0 ? (sumaPorMateria[mat.id] / countPorMateria[mat.id]).toFixed(1) : '-';
                return <td key={mat.id} className="border border-black p-1 text-center font-black text-blue-800">{promGral}</td>;
              })}
              <td className="border border-black p-1 bg-slate-200"></td>
            </tr>
            <tr className="bg-red-50">
              <td colSpan="2" className="border border-black p-1 text-right font-black uppercase text-red-700">Alumnos Reprobados por Materia:</td>
              {materias.map(mat => (
                <td key={mat.id} className="border border-black p-1 text-center font-black text-red-700">{reprobadosPorMateria[mat.id]}</td>
              ))}
              <td className="border border-black p-1 bg-slate-200"></td>
            </tr>
          </tfoot>
