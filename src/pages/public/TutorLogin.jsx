        ...
        <div className="flex-1 w-full lg:max-w-xl text-white">
          <div className="mb-10 text-center lg:text-left">
            <h2 className="text-4xl lg:text-5xl font-extrabold drop-shadow-md">
              Esc. Sec. Téc. N°68 <br/><span className="text-sky-400">"RENACIMIENTO"</span>
            </h2>
          ...
          <div className="bg-slate-800/40 backdrop-blur-md rounded-2xl border border-slate-700/50 p-6 shadow-2xl">
            <h3 className="text-2xl font-bold flex items-center mb-6 text-white border-b border-slate-700/50 pb-4">
              <Megaphone className="mr-3 text-sky-400 h-6 w-6"/> Muro de Avisos
            </h3>
                ...
                avisos.map(aviso => (
                  <div key={aviso.id} className="bg-slate-800/60 hover:bg-slate-800/80 transition-colors backdrop-blur border border-slate-700 p-5 rounded-xl shadow-lg">
                    <div className="flex items-center space-x-2 mb-3">
                      {aviso.type === 'warning' && <AlertTriangle className="h-4 w-4 text-indigo-400" />}
                      {aviso.type === 'success' && <CheckCircle className="h-4 w-4 text-emerald-400" />}
                      {aviso.type === 'info' && <Info className="h-4 w-4 text-sky-400" />}
                      
                      <span className={`px-2 py-0.5 rounded text-xs font-semibold ${
                        aviso.type === 'warning' ? 'bg-indigo-500/20 text-indigo-300 border border-indigo-500/30' : 
            ...
            <div className="p-8">
              ...
              <form className="space-y-6 animate-in fade-in" onSubmit={handleStudentLogin}>
                <div>
                  ...
                  <input type="text" placeholder="Ej. 2024EST68001 o CURP" required className="mt-1 w-full px-3 py-2 border rounded-lg focus:ring-sky-500 focus:border-sky-500 uppercase transition-shadow" value={identifier} onChange={e => setIdentifier(e.target.value.toUpperCase())} />
                </div>
                <button type="submit" disabled={loading} className="w-full flex justify-center py-2.5 px-4 rounded-lg text-sm font-bold text-white bg-sky-600 hover:bg-sky-700 disabled:opacity-50 transition-colors shadow-md">
                  {loading ? 'Buscando...' : 'Entrar a Mi Portal'}
                </button>
              </form>
              
              <div className="mt-6 text-center border-t pt-5">
                <a href="/" className="text-sm font-medium text-slate-500 hover:text-sky-600 transition-colors">
