...
let code = fs.readFileSync('src/components/HojaDeVida.jsx', 'utf8');
let lines = code.split(/\r?\n/);

// Find line: "        {/* Footer */}"
let footerIdx = lines.findIndex(l => l.includes('{/* Footer */}'));
// Two lines above footer should be "          )}"
// Three lines above should be "            </div>"
// Let's go up from footerIdx until we find "          )}"
let isEditingEndIdx = -1;
for (let i = footerIdx; i > 0; i--) {
    if (lines[i].includes(')}')) {
        isEditingEndIdx = i;
        break;
    }
...
// The line before isEditingEndIdx is the closing div of space-y-8.
let closingDivIdx = isEditingEndIdx - 1;
...
lines.splice(closingDivIdx, 0, newSection);

fs.writeFileSync('src/components/HojaDeVida.jsx', lines.join('\\n'));
console.log('HojaDeVida updated safely!');
